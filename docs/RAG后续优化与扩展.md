...
本文档整理当前项目 RAG 流程中仍然比较粗糙的地方，以及后续最值得推进的优化点。当前系统已经完成从 0 到 1：可以导入知识库、生成 embedding、执行 hybrid 检索、参与出题，并能通过调试脚本查看检索命中结果。下一阶段重点应放在可维护性、可评估性和资料导入能力上。

## 当前还比较粗糙的地方

### 1. 关键词检索仍然比较简单

当前关键词检索不是 BM25，也不是 PostgreSQL full-text search，而是基于文本包含关系和简单分数规则。

它可以满足开发阶段验证，但在数据变多后会有局限：

- 中文分词能力弱。
- 同义词、近义表达处理不足。
- 专有名词、英文缩写、数字、代码片段的权重还不够精细。
- 不同字段之间没有明确权重区分，例如标题、正文、tags、解释目前基本放在一起算。

后续如果知识库规模扩大，关键词检索需要升级为更稳定的检索策略。

### 2. 混合检索分数融合比较直接

当前 `hybrid-rag-v1` 的排序逻辑可以简化理解为：

```text
最终分数 = 关键词分数 + 向量分数
```

这种方式清晰、容易调试，但不够成熟。问题在于关键词分和向量分的分布不一定稳定，数据量变大后可能出现某一类分数长期压过另一类分数。
