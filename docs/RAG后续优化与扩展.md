...
当前关键词检索已经升级为 `hybrid-rag-v1.1`：PostgreSQL 环境优先使用 Full-Text Search，安装 `pg_jieba` 时使用 `jiebacfg` 做中文分词；如果扩展不可用，则自动降级到 `simple` FTS 和 Python 字段加权 scorer。
...
当前 `hybrid-rag-v1.1` 的排序逻辑可以简化理解为：
