...
- 产品名称：AI 闯关学习小程序
- 产品形态：微信小程序
- 当前阶段：MVP 已形成基础闭环，进入题库/RAG 增强与工程化迭代阶段
- 目标用户：希望用更轻松方式学习知识的普通用户，以及后续可扩展的垂直学习场景用户
- 核心一句话：用户输入一个学习主题或一段知识内容，小程序自动生成 AI 闯关题，优先复用自维护题库和 RAG 知识库，用户逐题答题并获得即时讲解，完成后得到复盘报告、历史记录和复训入口。
...
构建一个 AI 驱动的闯关学习微信小程序。当前产品主线聚焦“输入 -> 出题 -> 答题 -> 复盘 -> 沉淀 -> 复训”的学习闭环：

1. 用户输入学习主题或粘贴一段文本。
2. 系统优先从自维护题库/RAG 知识库召回相关题目和知识片段，不足时调用 AI 补齐题目。
3. 用户逐题答题。
4. 每题答完立即展示正误、正确答案和知识讲解。
5. 全部答完后生成通关总结和复盘报告。
6. 系统保存本次学习记录。
7. 用户可以回看历史记录、进入错题本复训，也可以继续扩展分享能力。

当前不追求覆盖所有知识输入形式。RAG 题库增强已经进入当前能力；文档、网页、截图、视频自动解析入库、知识库后台管理、AI 生图、多人 PK、排行榜、VIP 付费等仍作为后续扩展，避免范围失控。
...
- V1 不做多人实时竞技。
- 当前不做企业级知识库后台。
- 当前不做文档、网页、截图、视频的自动解析入库链路。
- V1 不做商业化闭环。
- V1 不做显式注册登录流程，用户身份采用微信静默登录方式获取。
...
系统根据用户输入生成结构化题目。当前已支持基于自维护题库/RAG 知识库增强出题：命中精选题时优先返回题库题，不足时把检索到的知识片段作为上下文交给 AI 补题。
...
- 题目应与用户输入主题或文本相关。
- 命中 RAG 时，题目应带有来源字段，便于后续分析题目来自精选题、RAG 补题还是普通 AI 生成。
...
系统在用户完成闯关并生成报告后保存学习记录，供后续回看。当前采用云端历史记录，不要求用户经过显式注册登录流程；小程序端通过微信静默身份换取后端会话。
...
V1 不做显式注册登录，但需要区分不同用户的云端历史记录。当前策略：

- 小程序端通过 `Taro.login` 获取微信临时 code。
- 后端调用微信 code2session 获取 openid，并签发本项目自己的访问 token。
- 开发环境允许在未配置 `WECHAT_APPID` / `WECHAT_SECRET` 时使用本地 fallback 身份，便于联调。
- 学习记录只在完成答题并生成报告后保存到 PostgreSQL。
- 当前学习会话仍保存在前端本地缓存中，用于答题过程恢复。
- 分享内容不应暴露用户敏感信息。
...
1. 题目数量 V1 固定为 5 题还是后续开放选择？
2. AI 是否允许联网搜索，还是 V1 只基于用户输入生成？
3. 复盘报告是否继续完全实时生成，还是增加本地模板兜底展示？
4. 分享落地页展示用户成绩，还是让好友重新闯关？
5. 是否需要在 V1 加入用户反馈题目错误？
6. 首发定位是通用学习工具，还是先选择一个垂直场景？

已决策：

- V1 不做强制显式登录，采用微信静默身份。
- 历史记录采用云端保存，底层使用 PostgreSQL。
- 当前只保存完成后的学习记录，不保存未完成答题进度。

## 17. Further Notes

当前产品的关键不是一次性做成“万能学习平台”，而是持续验证一个可沉淀的学习闭环：用户愿意输入学习主题，题库/RAG/AI 生成的题目能让用户完成闯关，逐题讲解能带来学习价值，通关复盘和错题复训能让用户愿意回来继续学习。

当前 RAG 题库增强已经落地为基础能力；后续再扩展到文档解析、网页解析、截图资料导入、视频解析、知识库后台、垂直题库、多人 PK、VIP 付费等方向会更稳。

当前建议的下一步是完善资料入库、检索评估、知识库管理和分享传播能力，同时继续优化首页输入、题目生成中状态、答题页、复盘页、历史记录页和错题复训体验。
